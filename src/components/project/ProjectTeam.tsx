...
interface ProjectTeamProps {
    autores?: TeamMember[]
    orientadores?: Advisor[] // Mantido para compatibilidade ou exibição simples
    historicoOrientadores?: Advisor[] // Nova prop para histórico completo
                                    ...
                                    <div key={idx} className={`flex flex-col md:flex-row items-center justify-between gap-4 p-4 rounded-2xl border shadow-sm relative overflow-hidden group ${isLider
                                        ...
                                        <div className="flex items-center gap-4 w-full md:w-auto flex-1 min-w-0">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className={`${isLider ? 'text-base' : 'text-sm'} font-bold text-gray-900 dark:text-white truncate`}>
                                                    {autor.nome}
                                                </p>
                                                {showEmail && (
                                        ...
                                        {showContactInfo && isLider && (
                                            <a
                                                ...
                                                className="flex items-center justify-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 text-blue-600 dark:text-blue-400 rounded-xl font-bold shadow-sm border border-blue-100 dark:border-blue-900 hover:scale-105 transition-transform whitespace-nowrap flex-shrink-0"
                                            >
                                ...
                                sortedAdvisors.map((orientador, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className={`flex flex-col md:flex-row items-center justify-between gap-4 p-4 rounded-2xl border shadow-sm transition-all ${isAtivo
                                            ...
                                            <div className="flex items-center gap-4 w-full md:w-auto flex-1 min-w-0">
                                                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-md border-2 border-white dark:border-gray-700 flex-shrink-0 ${isAtivo
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    {!isAtivo && orientador.removido_em && (
                                                        <p className="text-[10px] text-gray-500 mt-0.5">
                                                            Até {new Date(orientador.removido_em).toLocaleDateString('pt-BR', { month: 'short', year: 'numeric' })}
                                                        </p>
                                                    )}
                                                    {showEmail && isAtivo && orientador.email && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate">{orientador.email}</p>}
                                                </div>
                                            </div>

                                            {showContactInfo && isAtivo && orientador.email && (
                                                <a
                                                    href={`mailto:${orientador.email}`}
                                                    className="flex items-center justify-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 text-purple-600 dark:text-purple-400 rounded-xl font-bold shadow-sm border border-purple-100 dark:border-purple-900 hover:scale-105 transition-transform whitespace-nowrap flex-shrink-0"
                                                >
                                                    <Mail className="w-4 h-4" />
                                                    <span className="hidden sm:inline">Email</span>
                                                </a>
